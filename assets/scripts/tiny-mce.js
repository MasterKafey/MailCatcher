...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function addElementToEditor(content) {
        const element = document.createElement('div');
        element.innerHTML = content;
        const addedElement = element.firstChild;
        addedElement.classList.add('selectable-element');
        tinymce.activeEditor.execCommand('mceInsertContent', false, addedElement.outerHTML);
    }
    ...
    function openButtonModal() {
        ...
        var borderRadius = confirm("Voulez-vous un bouton avec des bords arrondis ?");
        var buttonContent = '<a href=' + link + ' style="background-color: ' + backgroundColor + '; border: solid ' + borderColor + ' 1px; color: ' + textColor + ';';
        if (borderRadius) {
            buttonContent += ' border-radius: 5px;';
        }
    ...
    function insertSeparator() {
        tinymce.activeEditor.execCommand('mceInsertContent', false, '<hr style="border: 0; height: 1px; background-color: black;">');
    }
    ...
    function addSpacer() {
        const spacer = '<div class="py-4"</div>';
        tinymce.activeEditor.execCommand('mceInsertContent', false, spacer);
    }

    // -----------------------------------------OPTION AJOUTER CODE HTML-----------------------------------------------------------------

    document.getElementById('htmlCodeButton').addEventListener('click', function (event) {
        event.preventDefault();
        addHTMLCode();
    });

    function addHTMLCode() {
        const htmlCode = '<div><p>Insérez votre code HTML ici</p></div>';
        tinymce.activeEditor.execCommand('mceInsertContent', false, htmlCode);
    }
    ...
    // -----------------------------------------OPTION AJOUTER MODELE FOOTER-----------------------------------------------------------------

    document.getElementById('footerButton').addEventListener('click', function (event) {
        event.preventDefault();
        insertFooter();
    });
    ...
    <div class="container px-6 py-8 mx-auto">
        <div class="flex flex-col items-center text-center">
            <a href="#">
                <img class="w-auto h-10" src="/images/logo.svg" alt="">
            </a>
    ...
    `;
        tinymce.activeEditor.execCommand('mceInsertContent', false, footerHTML);
    }
    ...
    document.getElementById('titleButton').addEventListener('click', function (event) {
        event.preventDefault();
        var titleContent = '<h1 class="font-bold text-xl">Nouveau titre</h1>';
        addElementToEditor(titleContent);
    });
    ...
    document.getElementById('paragraphButton').addEventListener('click', function (event) {
        event.preventDefault();
        var paragraphContent = '<p>Nouveau paragraphe</p>';
        addElementToEditor(paragraphContent);
    });
